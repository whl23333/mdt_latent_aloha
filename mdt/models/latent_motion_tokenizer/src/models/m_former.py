...
class MFormer3D(ViTPreTrainedModel):
    def __init__(self,
                 config: ViTConfig,
                 add_pooling_layer: bool = True,):
        super().__init__(config)
        ...
        # Initialize weights and apply final processing
        self.post_init()
        self.latent_motion_token = nn.Parameter(torch.zeros(1, self.query_num, config.hidden_size))
    ...
    def forward(
        ...
        # and head_mask is converted to shape [num_hidden_layers x batch x num_heads x seq_length x seq_length]
        head_mask = self.get_head_mask(head_mask, self.config.num_hidden_layers)
        latent_motion_tokens = self.latent_motion_token.expand(combined_tokens.size(0), -1, -1)
        embeddings = torch.cat((latent_motion_tokens, combined_tokens), dim=1)

        encoder_outputs = self.encoder(embeddings, head_mask=head_mask, output_attentions=output_attentions,
