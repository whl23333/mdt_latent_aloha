...
import transformers
from .m_former import SymmetricCrossAttention
...
class LatentMotionTokenizer3D_Simple(nn.Module):
    def __init__(
            self,
            # image_encoder,
            m_former,
            ...
            flow_checkpoint_path=None,
            compute_flow_kwargs: Optional[Dict[str, Any]] = None,
            # parameters for symmetric cross attention
            cross_dim=768,
    ):
        super().__init__()
        ...
        self.m_former = m_former
        # Symmetric cross attention for 3D fusion
        self.symmetric_cross_attention = SymmetricCrossAttention(
            dim=cross_dim,
            dropout=0.0,
            batch_first=True,
            fuse='mean'
        ...
        self.vq_down_resampler = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(cross_dim, codebook_embed_dim)
        )
        self.vq_up_resampler = nn.Sequential(
            nn.Linear(codebook_embed_dim, codebook_embed_dim),
            nn.Tanh(),
    ...
    def tokenize(self, cond_pixel_values1, target_pixel_values1, 
        ...
        # 融合为3D运动token
        # latent_3d_motion_tokens = self.m_former3d(combined_tokens).last_hidden_state[:, :self.m_former3d.query_num]
        latent_3d_motion_tokens = self.symmetric_cross_attention(
            latent_motion_tokens1,
            latent_motion_tokens2
        ) # (b, per_len, cross_dim)
...
class LatentMotionTokenizer3D_Simple_Sngl_Dcdr(nn.Module):
    def __init__(
            self,
            # image_encoder,
            m_former,
    ...
    def decode_image(self, cond_pixel_values, given_motion_token_ids, **kwargs):
        ...
        recons_pixel_values = self.decoder(cond_input=cond_pixel_values, 
                                           latent_motion_tokens=latent_motion_tokens_up, 
                                           **kwargs)
        return {
            "recons_pixel_values": recons_pixel_values,
    ...
    @torch.no_grad()
    def embed(self, cond_pixel_values, target_pixel_values, before_vq=False, pool=False, avg=False):
        # 首先提取视角的运动token
        latent_motion_tokens = self._get_view_motion_tokens(cond_pixel_values, target_pixel_values)
    ...
    def tokenize(self, cond_pixel_values1, target_pixel_values1, 
        ...
        """
        # 提取视角1的运动token
        latent_motion_tokens1 = self._get_view_motion_tokens(cond_pixel_values1, target_pixel_values1)
        
        # 提取视角2的运动token
        latent_motion_tokens2 = self._get_view_motion_tokens(cond_pixel_values2, target_pixel_values2)
    ...
    def _get_view_motion_tokens(self, cond_pixel_values, target_pixel_values):
        """
        提取单视角的运动token(内部方法)
        """
        with torch.no_grad():
    ...
    def forward(self, 
        ...
        # 提取视角1的运动token
        latent_motion_tokens1 = self._get_view_motion_tokens(
            cond_pixel_values1, target_pixel_values1)  # (b, q, d)
        
        # 提取视角2的运动token
        latent_motion_tokens2 = self._get_view_motion_tokens(
            cond_pixel_values2, target_pixel_values2)  # (b, q, d)
        ...
        # ================== 第四阶段：双视角重建 ==================
        # 使用共享的3D运动token重建两个视角
        recons_pixel_values1 = self.decoder(
            cond_input=cond_pixel_values1,
            latent_motion_tokens=latent_3d_motion_up,
            **kwargs
        )  # (b, c, h, w)

        recons_pixel_values2 = self.decoder(
            cond_input=cond_pixel_values2,
            latent_motion_tokens=latent_3d_motion_up,
            **kwargs
        )  # (b, c, h, w)
